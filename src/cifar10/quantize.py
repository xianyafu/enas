...
def Quantize(graph,
  ...
  for op in (op for op in graph_ops if _IsInterestingOpWithWeights(op)):
    if op.name.endswith('/depthwise') and op.name in qint_ops:
      ...
      if separable_conv and separable_conv.type == 'Conv2D':
        continue
    if (not op.name.endswith('_Fold')) and (op.name in qint_ops):
    ...
    elif op.name.endswith('_Fold') and (op.name in qint_ops):
      print('no fold...........', op.name)
...
class _QuantizeContext(object):
    ...
    if folded:
      ...
      try:
        input_idx = next(i for i, v in enumerate(op.inputs)
                         if '/w_qint8' in v.name or
                         '/transpose_1' in v.name) #or
                         #'FakeQuantWithMinMaxVars' in v.name)
  ...
  def _GetReluAndAddOperations(self, context, op):
    ...
    """
    activation_op = self.graph.get_operation_by_name(context+'/'+'Relu')#common.GetEndpointActivationOp(self.graph, context)
    if activation_op:
