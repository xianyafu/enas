...
def get_train_ops(
  ...
  """
  with tf.variable_scope("get_train_op",reuse=tf.AUTO_REUSE):
    if l2_reg > 0:
      l2_losses = []
      for var in tf_variables:
    ...
    if lr_cosine:
      ...
      #                         name="last_reset")
      last_reset = tf.get_variable(shape=[], dtype=tf.int32,trainable=False,
                     name="last_reset", initializer = tf.constant_initializer(0))
      T_i = tf.get_variable(shape=[], dtype=tf.int32, trainable=False, name="T_i",
                     initializer = tf.constant_initializer(lr_T_0))
