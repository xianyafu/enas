...
class GeneralController(Controller):
  ...
  def build_trainer(self, child_model):
    ...
    self.in_time = child_model.infer_time
    self.time = tf.reduce_sum((tf.to_float(child_model.infer_time)/
                tf.to_float(tf.constant(0.025))))
    w1 = tf.cond(tf.less(self.mem, tf.constant(1.0)), lambda:tf.constant(0), lambda:tf.constant(-1))
    w2 = tf.cond(tf.less(self.time, tf.constant(1.0)), lambda:tf.constant(0), lambda:tf.constant(-1))
    self.reward = self.valid_acc #* tf.pow(self.mem,tf.to_float(w1)) * tf.pow(self.time, tf.to_float(w2))
